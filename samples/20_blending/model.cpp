...
Model::Model(const std::string& path, unsigned int start_tex_id) : m_start_tex_id(start_tex_id)
{
...
void Model::draw(Renderer& renderer, Shader& shader, bool use_tex)
{
    size_t mesheSize = m_meshes.size();
    for (unsigned int i = 0; i < mesheSize; ++i)
    {
        m_meshes[i]->draw(renderer, shader, use_tex);
    }
}
...
Mesh *Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    return new Mesh(vertices, indices, textures, m_start_tex_id);
}
